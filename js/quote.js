  ...
  async function submitQuote(client) {
    ...
    if (!WEBHOOK_URL || WEBHOOK_URL === 'REPLACE_ME_WITH_YOUR_WEBHOOK_URL') {
      ...
      setStatus('success',
        'Your quote is ready',
        `Your PDF quote (reference <strong>${escape(payload.reference)}</strong>) has downloaded to this device. Please email it to <a href="mailto:${SALES_EMAIL}" style="color:var(--ink); text-decoration: underline;">${SALES_EMAIL}</a> or call +27 64 521 0662 — a consultant will be in touch within one business day.`);
      return;
    }
    ...
    try {
      ...
      setStatus('success',
        ...
        `Your PDF (reference <strong>${escape(payload.reference)}</strong>) has downloaded to this device, and we've notified our sales team. A consultant will be in touch within one business day at <strong>${escape(client.email)}</strong>.`);
    } catch (err) {
  ...
  function showQuote(client) {
    ...
    [
      ...
      c.innerHTML = `<div class="ck-label">${label}</div><div class="ck-val">${escape(val)}</div>`;
      cc.appendChild(c);
    });
